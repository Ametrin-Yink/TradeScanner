...
def test_table_has_8_columns(tmp_path):
    """Active Setups table should have 8 columns: Symbol, R:R, Setup, Price, Entry+Dist, Stop, Target, RS."""
    market = MarketOverview(
        date='2026-06-19', regime='bull_moderate', confidence=70,
        reasoning='Market is steady.', spy_price=525.0, spy_change_5d=1.2,
        vix=14.5, vix_status='low',
    ...
    gen = ReportGenerator(reports_dir=tmp_path)
    content = open(gen.generate_report(result)).read()
    table_header = '<th>Symbol</th><th>R:R</th><th>Setup</th><th>Price</th><th>Entry+Dist</th><th>Stop</th><th>Target</th><th>RS</th>'
    assert table_header in content, "Table header should have R:R-first column order"
