...
class StrategyScreener:
    ...
    def _precalculate_rs_scores(self, symbols: List[str]) -> Dict[str, Dict]:
        """
        Pre-calculate RS scores for all symbols once.
        RS = 0.4 * ret_3m + 0.3 * ret_6m + 0.3 * ret_12m

        Returns:
            Dict mapping symbol to RS data including percentile ranking
        """
        logger.info(f"Pre-calculating RS scores for {len(symbols)} symbols...")
        rs_data = {}

        for symbol in symbols:
            try:
                df = self._get_data(symbol)
                if df is None or len(df) < 60:  # Need at least 60 days for 3m return
                    continue
        ...
        if not rs_data:
            logger.warning("No RS data calculated")
            return {}

        # Calculate percentile rankings
        all_scores = [d['rs_score'] for d in rs_data.values()]
        all_scores.sort()
        n = len(all_scores)

        for symbol, data in rs_data.items():
            # Calculate percentile (how many scores are below this one)
            below_count = sum(1 for s in all_scores if s < data['rs_score'])
            data['percentile'] = (below_count / n) * 100 if n > 0 else 50

        logger.info(f"Calculated RS scores for {len(rs_data)} symbols")
        return rs_data
    ...
    def screen_all(
        ...
        # Clear indicator cache before starting new screening session
        TechnicalIndicators.clear_cache()
        logger.info("Cleared indicator cache for new screening session")

        # Pre-calculate RS scores for all symbols (shared across strategies)
        self._rs_cache = self._precalculate_rs_scores(symbols)

        # Load earnings if not already loaded
        if not self.earnings_calendar:
            self.load_earnings_calendar(symbols)

        # Share market data, earnings calendar, and RS cache with all strategies
        for strategy in self._strategies.values():
            strategy.market_data = self.market_data
            strategy.rs_cache = self._rs_cache  # Share RS data
        ...
        cache_stats = TechnicalIndicators.get_cache_stats()
        logger.info(f"Indicator cache stats: {cache_stats}")
